...
function extractWeightRegex(text) {
    ...
    // Current regex in fix-weights.js (No 'l' or 'л')
    ...
    for (const m of matches) {
        ...
        const fullMatch = m[0];
        console.log(`Match: ${fullMatch} -> ${val} ${unit}`);
...
async function run() {
    ...
    console.log(`Current DB Weight: ${p.weight}kg`);
    console.log(`Current AI Weight: ${p.aiWeight}kg`);
    console.log(`Current AI Reason: ${p.aiWeightReason}`);

    const text = getProductText(p);
    const result = extractWeightRegex(text);
    console.log("New Regex Result:", result);
}
