...
import { getProductWeight, calculateFinalPrice, getPriceBreakdown } from '../utils/productUtils';
...
export default function ProductDetail() {
    ...
    // Add to cart is handled inline by shipping buttons below
                    ...
                    <div>
                                        ...
                                        >
                                            <div className="flex justify-between items-center font-bold text-gray-800 text-sm">
                                                ...
                                                <span className={`text-xl ${type === 'ground' ? 'text-blue-600' : 'text-orange-600'}`}>
                                                    Нийт : {breakdown.finalMNT.toLocaleString()}₮
                                                </span>
                                            </div>
                                            <div className="text-gray-600 text-[11px] sm:text-xs text-left space-y-0.5">
                                                <div>Барааны үнэ: <span className="font-semibold text-gray-800">{breakdown.baseDisplay}</span></div>
                                                <div>Тээвэр: <span className="font-semibold text-gray-800">{breakdown.shippingDisplay}</span> <span className="text-gray-400">({breakdown.weightDisplay} x {breakdown.rateDisplay})</span></div>
                                                <div className="font-bold text-gray-900">Нийт: {breakdown.totalDisplay}</div>
                                            </div>
                                        </button>
                                    );
                                })}
                        ...
                        <div className="flex flex-col gap-3 mb-8">
                            ...
                            <button
                                ...
                                className="w-full bg-white text-red-600 border-2 border-red-600 px-6 py-4 rounded-lg font-bold hover:bg-red-50 transition-all duration-200 flex items-center justify-center gap-3 text-lg shadow-sm active:scale-95"
                            >
