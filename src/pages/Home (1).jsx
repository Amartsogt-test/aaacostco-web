...
export default function Home() {
    ...
    const {
        ...
        searchTerm, setSearchTerm, isSearching, searchProducts
    } = useProductStore();
    ...
    // Reset displayCount when products change (new filter/search)
    useEffect(() => {
        ...
        const observer = new IntersectionObserver(
            (entries) => {
                if (entries[0].isIntersecting) {
                    setDisplayCount((prev) => Math.min(prev + LOAD_INCREMENT, 50));
                }
            },
            { threshold: 0.1 }
        );
    ...
    const { currentCategory, currentSubCategory } = useProductStore();
    ...
    // Compute available tags from all products
    const availableTags = useMemo(() => {
        const tags = new Set();
        products.forEach(p => {
            if (p.additionalCategories && Array.isArray(p.additionalCategories)) {
                p.additionalCategories.forEach(tag => {
