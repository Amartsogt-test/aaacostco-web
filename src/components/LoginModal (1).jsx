...
const LoyaltyCard = React.lazy(() => import('./LoyaltyCard'));

export default function LoginModal({ isOpen, onClose, onSuccess }) {
    const navigate = useNavigate();
    const [phoneNumber, setPhoneNumber] = useState('');
    const [verificationCode, setVerificationCode] = useState('');
    const [phoneStep, setPhoneStep] = useState('input'); // 'input', 'sending', 'verifying', 'facebook-connect', 'success'
    ...
    useEffect(() => {
        ...
        const initVerifier = async () => {
            if (isOpen && recaptchaRef.current && !verifierRef.current) {
                try {
    ...
    const requestOtp = async (e) => {
        ...
        // const resultPhoneNumber = formattedPhone; // Unused
    ...
    const verifyOtp = async (e) => {
        ...
        try {
            ...
            try {
                // We attempt to read/write DB even for bypass users as requested
                ...
                // FALLBACK: If DB fails but user is valid bypass admin, let them in matching their role!
                const ADMIN_NUMBERS = ['+976' + ADMIN_PHONE];

                if (result.user.uid.startsWith('admin-bypass-') || ADMIN_NUMBERS.includes(result.user.phoneNumber)) {
                    console.log("✅ Using fallback Admin status (DB write skipped due to permission)");
                    userData.isAdmin = true;
                    // Proceed to login with memory-only admin status
                } else {
                    // For standard users, we might technically let them in as non-admin or show error,
                    // but here we just proceed with default userData (isAdmin: false)
                    console.log("⚠️ Logging in with default permissions due to DB error");
                }
            }

            login(userData);

            // Redirect Admin to Profile immediately
            if (userData.isAdmin) {
                navigate('/profile');
            }

            setPhoneStep('success'); // Intermediate success state

            // Check if admin - if so, skip facebook connect
            // MODIFIED: We want admins to see Facebook Connect too, so we comment this out
            /* 
            if (userData.isAdmin) {
                login(userData);
                navigate('/profile');
            ...
            */
            ...
            // For normal users, move to Facebook Connect step
            // We temporarily login the user state but keep modal open
            login(userData);
    ...
    const handleFacebookConnect = async () => {
        try {
            ...
            // ADMIN BYPASS: If test user 23568947
            // Check both phone AND uid since admin-bypass accounts may have phone: null
            const isAdminBypass = authUser?.phone?.includes('23568947') || authUser?.uid?.includes('23568947');
            if (isAdminBypass) {
                console.log("⚠️ TEST USER (Modal): Simulating Facebook Link Success");
                const newData = {
                    name: 'Bilguun Admin',
            ...
            // Update Local Store (Need to re-login to merge new data)
            const { user: currentUser } = useAuthStore.getState(); // get current state
    ...
    return (
        <div className="fixed inset-0 z-[55] bg-white overflow-y-auto pb-[140px]">
            <div className="min-h-full flex flex-col">
                {/* Header */}
                <div className="bg-costco-blue px-8 py-6 text-center relative shrink-0">
                    ...
                    {isAuthenticated ? (
                        <div className="space-y-6 animate-fade-in">
                            ...
                            <div className="text-center">
                                <p className="text-gray-600 font-medium">Та хэдийн нэвтэрсэн байна.</p>
                                <button
                                    onClick={onClose}
                                    className="mt-4 px-6 py-2 bg-costco-blue text-white rounded-lg font-bold hover:bg-blue-700 transition"
                                >
                                    Үргэлжлүүлэх
                                </button>
                            </div>
                        ...
                        <>
                            ...
                            {error && (
                                <div className="bg-red-50 text-red-500 text-sm p-3 rounded-lg text-center mb-4">
                                    {error}
                                </div>
                            )}
                            ...
                            {phoneStep === 'input' || phoneStep === 'sending' ? (
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="relative">
                                            <input
                                                ...
                                                maxLength={8}
                                                className="w-full pl-12 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-costco-blue focus:border-costco-blue outline-none transition font-sans"
                                            />
                                ...
                                <div className="space-y-6 animate-fade-in">
                                    ...
                                    <button
                                        ...
                                        className={`w-full py-3.5 rounded-xl font-bold transition shadow-lg flex items-center justify-center gap-2 ${verificationCode.length === 6 && !isVerifying ? 'bg-green-600 text-white hover:bg-green-700 shadow-green-200' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}
                                    >
                                        {isVerifying ? (
                                            <>
                                                <RefreshCw className="animate-spin" size={20} />
                            ...
                            {phoneStep === 'facebook-connect' && (
                                <div className="space-y-6 animate-fade-in text-center">
                                    <div className="w-16 h-16 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-4">
                                        <RefreshCw size={28} className="animate-none" /> {/* Or checkmark */}
                ...
                <div className="bg-gray-50 border-t border-gray-100 p-10">
