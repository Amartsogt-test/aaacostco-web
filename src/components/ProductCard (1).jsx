...
const ProductCard = memo(function ProductCard({ product, isFeatured }) {
    ...
    const isInGround = useCartStore(state => state.groundItems.some(item => item.id === product.id));
    const isInAir = useCartStore(state => state.airItems.some(item => item.id === product.id));
                        ...
                        <div className="flex gap-3 w-full justify-center mt-1">
                            ...
                            <button
                                onClick={(e) => {
                                    ...
                                    e.stopPropagation();
                                    if (isInGround) {
                                        useCartStore.getState().removeFromGround(product.id);
                                        showToast('Газраар сагснаас хасагдлаа', 'info');
                                    } else {
                                        useCartStore.getState().addToGround(product);
                                        showToast('Газраар сагсанд нэмэгдлээ', 'success');
                                    }
                                }}
                                className={`flex-1 aspect-square max-w-[40px] rounded-xl flex flex-col items-center justify-center transition ${isInGround
                                    ? 'bg-blue-100 text-blue-600 hover:bg-blue-200'
                                    : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                                title={isInGround ? "Газраар сагснаас хасах" : "Газраар авах"}
                            >
                            ...
                            <button
                                onClick={(e) => {
                                    ...
                                    e.stopPropagation();
                                    if (isInAir) {
                                        useCartStore.getState().removeFromAir(product.id);
                                        showToast('Агаараар сагснаас хасагдлаа', 'info');
                                    } else {
                                        useCartStore.getState().addToAir(product);
                                        showToast('Агаараар сагсанд нэмэгдлээ', 'success');
                                    }
                                }}
                                className={`flex-1 aspect-square max-w-[40px] rounded-xl flex flex-col items-center justify-center transition ${isInAir
                                    ? 'bg-orange-100 text-orange-600 hover:bg-orange-200'
                                    : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                                title={isInAir ? "Агаараар сагснаас хасах" : "Агаараар авах"}
                            >
